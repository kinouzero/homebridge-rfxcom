...
import { PLUGIN_NAME, MODE } from './settings';
...
export class SwitchAccessory {
  ...
  constructor(
    ...
    remote.accessory.getService(remote.platform.Service.AccessoryInformation)!
      ...
      .setCharacteristic(remote.platform.Characteristic.SerialNumber,
        `${remote.rfyRemote.deviceID}-${remote.device.unitCode}-${direction}`);

    this.service = remote.accessory.getService(remote.platform.Service.Switch)
      || remote.accessory.addService(remote.platform.Service.Switch);
  ...
  async setOn(value: CharacteristicValue) {
    ...
    if (this.remote.platform.debug) {
      this.remote.platform.log.debug(`Remote ${this.context.deviceID}:  Set ${this.context.name}, on=${value}.`);
    }
  }
