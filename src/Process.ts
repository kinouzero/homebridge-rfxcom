...
export class Process {
   ...
   */
   start() {
    ...
    if(this.remote.context.process) this.stop();
    ...
    // Start new process
    if([DIRECTION.up, DIRECTION.down].includes(this.remote.context.direction)) this.remote.context.process = setInterval(() => this.processing(), 1000);

    if(this.debug) this.log(`this.remote ${this.remote.rfyRemote.deviceID}: Starting ${_shutter.context.name}, direction=${this.remote.context.direction}, currentPosition=${_shutter.context.currentPosition}`);
    else this.log(`this.remote ${this.remote.rfyRemote.deviceID}: Processing RFY ${this.remote.context.direction}...`);
  }
  ...
  stop() {
    let _shutter: ShutterAccessory;
    if(!this.remote || !(_shutter = this.remote.shutter)) return;

    // Stop process
    clearInterval(this.remote.context.process);
  
    // Set direction to stop
    if(this.remote.context.direction === DIRECTION.stop) return;
    this.remote.context.direction = DIRECTION.stop;
  
    // Reset switches
    for(const d in this.remote.switches) this.remote.switches[d].setOn(false);
    ...
    // RFY Command Stop
    if(_shutter.context.currentPosition < 100 && _shutter.context.currentPosition > 0) this.rfy.stop(this.remote.rfyRemote.deviceID);
  
    if(this.debug) this.log(`this.remote ${this.remote.rfyRemote.deviceID}: Stopping ${_shutter.context.name}, currentPosition=${_shutter.context.currentPosition}`);
  }
  setSwitch(arg0: any, arg1: boolean) {
  ...
  processing() {
    ...
    if(this.remote.context.direction === DIRECTION.up) value += (100 / this.remote.context.duration);
    if(this.remote.context.direction === DIRECTION.down) value -= (100 / this.remote.context.duration);
    _shutter.setCurrentPosition(value);
  
    if(this.debug) this.log(`this.remote ${this.remote.rfyRemote.deviceID}: Processing ${_shutter.context.name}, currentPosition=${_shutter.context.currentPosition}`);
    else this.log(`this.remote ${this.remote.rfyRemote.deviceID}: Stopping RFY ${this.remote.context.direction}`);
